...
import { AnalyticsContext, useApi, useRouteRef, useAnalytics } from '@backstage/core-plugin-api';
import qs from 'qs';
...
import { OngoingTaskContextMenu } from './OngoingTaskContextMenu.esm.js';
...
function OngoingTaskAnalyticsContext(props) {
...
const OngoingTask = (props) => {
  return /* @__PURE__ */ jsx(OngoingTaskAnalyticsContext, { children: /* @__PURE__ */ jsx(Page, { themeId: "website", children: /* @__PURE__ */ jsx(OngoingTaskChrome, { ...props }) }) });
};
function OngoingTaskBody(props) {
  const { taskId } = useParams();
  const taskStream = useTaskEventStream(taskId);
  ...
  return /* @__PURE__ */ jsxs(OngoingTaskAnalyticsContext, { children: [
    /* @__PURE__ */ jsx(
      OngoingTaskContextMenu,
      {
        ...
        isCancelButtonDisabled: isCancelButtonDisabled || cancelStatus !== "not-executed"
      }
    ),
    /* @__PURE__ */ jsx(
      OngoingTaskContent,
...
function OngoingTaskContent(props) {
  ...
  const buttonBarVisible = props.buttonBarVisibleOverride ?? buttonBarVisibleLocal;
  const setButtonBarVisibleState = props.onToggleButtonBar ?? setButtonBarVisibleStateLocal;
  const { allowed: canCancelTask } = usePermission({
    permission: taskCancelPermission,
    resourceRef: taskId
  });
  ...
  const cancelEnabled = !(taskStream.cancelled || taskStream.completed);
  return /* @__PURE__ */ jsxs(Content, { className: classes.contentWrapper, children: [
...
export { OngoingTask, OngoingTaskBody };
