...
var JsonSchemaGenerator = (function () {
    ...
    JsonSchemaGenerator.prototype.getDefinitionForRootType = function (propertyType, reffedType, definition, defaultNumberType, ignoreUndefined) {
        ...
        if (tupleType) {
            ...
            var targetTupleType_1 = propertyType.target;
            var fixedTypes = elemTypes.map(function (elType, index) {
                var _a, _b, _c;
                var def = _this.getTypeDefinition(elType);
                var label = (_c = (_b = (_a = targetTupleType_1.labeledElementDeclarations) === null || _a === void 0 ? void 0 : _a[index]) === null || _b === void 0 ? void 0 : _b.name) === null || _c === void 0 ? void 0 : _c.getFullText().trim();
                if (label) {
                    def.title = label;
                }
                return def;
            ...
            definition.minItems = targetTupleType_1.minLength;
            if (targetTupleType_1.hasRestElement) {
                definition.additionalItems = fixedTypes[fixedTypes.length - 1];
                fixedTypes.splice(fixedTypes.length - 1, 1);
            }
            else {
                definition.maxItems = targetTupleType_1.fixedLength;
            }
        }
        else {
    ...
    JsonSchemaGenerator.prototype.getDefinitionForProperty = function (prop, node) {
        ...
        if (valDecl === null || valDecl === void 0 ? void 0 : valDecl.initializer) {
            ...
            if (initial.expression) {
                ...
                try {
                    var vm = new VM();
                    var val = vm.run("sandboxvar=" + initial.getText());
                    if (val === null ||
                        typeof val === "string" ||
                        typeof val === "number" ||
                        typeof val === "boolean" ||
    ...
    JsonSchemaGenerator.prototype.getTypeDefinition = function (typ, asRef, unionModifier, prop, reffedType, pairedSymbol, forceNotRef, ignoreUndefined) {
        ...
        if (this.recursiveTypeRef.get(fullTypeName) === definition) {
            ...
            if (this.reffedDefinitions[fullTypeName]) {
                ...
                returnedDefinition = __assign({ $ref: createRefURI(this.args.id, fullTypeName) }, annotations);
            }
        }
...
function createRefURI(id, name) {
    ...
    return "".concat(id, "#/definitions/").concat(encoded);
}
